<template>
    <div class="mb-8 pb-8 border-b border-gray-400">
        ...
        <router-link class="text-sm text-sky-500" :to="{name: 'user.show', params: {id: post.user.id}} ">{{ post.user.name }}</router-link>
        ...
        <div v-if="is_repost" class="mt-4">
            ...
            <div>
                ...
                hover:bg-white hover:border hover:border-green-600 hover:text-green-600 box-border ml-auto">Опубликовать</a>
            </div>
        </div>

        <div v-if="post.comments_count > 0" class="mt-4">

            <p class="cursor-pointer" v-if="!isShowed" @click="getComments(post)">Показать коментарии ({{ post.comments_count }})</p>
            <p class="cursor-pointer" v-if="isShowed" @click="isShowed = false">Закрыть комментарии</p>
        ...
        <div class="mt-4">
            <div class=" mb-3">
                ...
                <p v-if="comment" @click="comment = null" class="cursor-pointer text-sky-400 text-sm">Отменить</p>
                </div>
            ...
            <div>
                ...
                hover:bg-white hover:border hover:border-green-600 hover:text-green-600 box-border ml-auto">Комментарий</a>
            </div>
        </div>
    </div>
</template>
